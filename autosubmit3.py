...
#                WARNING: the script does not check if all parameters
...
#                in the wetterturnier somewhere in the future
#                (not planned yet).
# -------------------------------------------------------------------
...
class read_file_content( object ):
   ...
   def summary(self):
      ...
      if self.belated:
         print("    - Tournament Date %s" % self.tdate)
...
if __name__ == "__main__":
   ...
   if not options.dryrun:
      ...
      elif sub.returncode == 5:
         print("SSL problem, 'maybe try --no-check-certificate' ?")
      else:
